...
public final class Banner {
    ...
    /// Shared instance.
    public static let shared = Banner()
    
    /// Margin between banner's and `safeAreaLayoutGuide` top.
    public static var topMargin: CGFloat = 5
    ...
    public private(set) weak var currentBanner: BannerView? {
        didSet {
     ...
     */
    public func present(configurationHandler: @escaping (BannerView) -> Void, duration: TimeInterval = 0.56, dismissAfter delay: TimeInterval = 5, in window: UIWindow? = nil, feedbackStyle: UIImpactFeedbackGenerator.FeedbackStyle? = nil, pressHandler: (() -> Void)? = nil, completionHandler: (() -> Void)? = nil) {
    ...
    /**
     ...
     - Parameters:
        ...
        - completionHandler: Action performed after banner disappears from the screen.
     */
    public func forcePresent(configurationHandler: @escaping (BannerView) -> Void, duration: TimeInterval = 0.56, dismissAfter delay: TimeInterval = 5, in window: UIWindow? = nil, feedbackStyle: UIImpactFeedbackGenerator.FeedbackStyle? = nil, pressHandler: (() -> Void)? = nil, completionHandler: (() -> Void)? = nil) {
     ...
     */
    public func update(configurationHandler: (BannerView) -> Void) {
    ...
    private func present(snapshot: Snapshot) {
        ...
        finalConstraint = view.topAnchor.constraint(equalTo: layoutGuide.topAnchor, constant: Banner.topMargin)
     ...
     */
    private func clean(after view: BannerView) {
...
internal extension Banner {
    /// Helper container class to keep configuration of the banner.
    class Snapshot {
        let identifier: String
        let configurationHandler: ((BannerView) -> Void)?
        ...
        init(configurationHandler: ((BannerView) -> Void)?, duration: TimeInterval, dismissAfter delay: TimeInterval, in window: UIWindow?, feedbackStyle: UIImpactFeedbackGenerator.FeedbackStyle?, pressHandler: (() -> Void)?, completionHandler: (() -> Void)?) {
